...
| Gestion des utilisateurs (suspension, rôle, reset) | ⏳ Partiel | Actions basiques dans admin |
| Modération des prédictions (file de validation) | ❌ Non fait | |
| Gestion des signalements (reports) | ❌ Non fait | Table `reports` en BDD |
...
| Journal d'audit des actions admin | ❌ Non fait | Table `admin_logs` en BDD |
| Export avancé (CSV, Excel, PDF) | ⏳ Partiel | CSV basique ajouté |
...
### Priorité Haute
1. **Tailwind CSS via PostCSS** (remplacer le CDN utilisé en développement — warning en console)
2. **Remplacer les `alert()` natifs** par des toasts/notifications stylisées (UX bien meilleure)
...
7. **Export PDF des analyses financières** (valeur ajoutée pour les utilisateurs pro)
8. **Mode de comparaison de portefeuilles** (comparer son évolution vs les pairs de sa cohorte)

    ...
    useEffect(() => {
        const fetchUsers = async () => {
            try {
                ...
                const usersWithCalculatedData = allUsers
                    .filter(user => user.role !== 'admin') // Exclude admin user from the list
                    .map(user => {
                        const holdingsValue = user.portfolio.holdings.reduce((acc, h) => acc + (h.shares * h.currentValue), 0);
